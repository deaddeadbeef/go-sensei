...
describe('local question answer', () => {
  ...
  it('turns resignation questions into a concrete recovery target', () => {
    ...
    expect(answer?.text).toContain('For guided learning, first take one useful idea from the board.');
    expect(answer?.text).toContain('Your current recovery job is: Start with a corner.');
    expect(answer?.text).toContain('Place your next stone near an empty corner. Try C7, G7, C3, or G3.');
    expect(answer?.text).toContain('play one of them before deciding to restart');
  ...
  it('answers start-over questions with the current post-move target', () => {
    ...
    expect(answer?.text).toContain('Your current recovery job is: Make your stones work together.');
    expect(answer?.text).toContain('Play a one-space jump from one of your stones. Try E7 or C5.');
    expect(answer?.boardFocus?.suggestions?.map((suggestion) => suggestion.point)).toEqual([
      { x: 4, y: 2 },
      { x: 2, y: 4 },
  ...
  it('answers shape questions after H8 as fresh-area extension practice', () => {
    ...
    expect(answer?.text).toContain('On this board, H8 is your anchor. Extend H8 into the upper-right area: try H6 or F8.');
    ...
    expect(answer?.boardFocus?.suggestions?.slice(0, 2)).toEqual([
      {
        id: 'local-shape-move-7,3',
        point: { x: 7, y: 3 },
        rank: 1,
        reason: 'Try H6 to give H8 a partner in the upper-right area while keeping a one-space gap.',
      },
      {
        id: 'local-shape-move-5,1',
        point: { x: 5, y: 1 },
        rank: 2,
        reason: 'Try F8 to give H8 a partner in the upper-right area while keeping a one-space gap.',
      },
  ...
  it('answers territory questions after H8 with fresh-area extension targets', () => {
    ...
    expect(answer?.text).toContain('I marked H6 and F8 because they extend H8 into the upper-right area: they help the fresh stone sketch a loose border without touching too closely.');
    expect(answer?.conceptIds).toEqual(expect.arrayContaining(['territory', 'shape', 'direction-of-play']));
    expect(answer?.boardFocus?.suggestions?.slice(0, 2)).toEqual([
      {
        id: 'local-territory-move-7,3',
        point: { x: 7, y: 3 },
        rank: 1,
        reason: 'Try H6 to give H8 a partner in the upper-right area while keeping a one-space gap.',
      },
      {
        id: 'local-territory-move-5,1',
        point: { x: 5, y: 1 },
        rank: 2,
        reason: 'Try F8 to give H8 a partner in the upper-right area while keeping a one-space gap.',
      },
  ...
  it('does not ask for a first Black stone when reading White from a restored study board', () => {
    const answer = getLocalQuestionAnswer('What can White do?', settledShapeGame(), 'guided');

    expect(answer?.text).toContain('This restored study position has board stones but no move history, so I cannot tell which Black stone White is answering.');
    expect(answer?.text).toContain('Use the current target first: Choose a new area.');
    expect(answer?.text).toContain('Try H8 or H2.');
    expect(answer?.text).toContain('I marked the current targets so the reply question has a real anchor.');
    expect(answer?.text).not.toContain('Play a Black stone first');
    expect(answer?.text).not.toContain('first targets');
    expect(answer?.conceptIds).toEqual(expect.arrayContaining(['reading', 'direction-of-play', 'shape']));
    expect(answer?.boardFocus?.suggestions).toEqual([
      {
        id: 'local-white-reply-move-7,1',
        point: { x: 7, y: 1 },
        rank: 1,
        reason: 'Consider H8 as a fresh upper-right direction away from the settled local shape.',
      },
      {
        id: 'local-white-reply-move-7,7',
        point: { x: 7, y: 7 },
        rank: 2,
        reason: 'Consider H2 as a fresh lower-right direction away from the settled local shape.',
      },
  ...
  it('does not ask for a first Black stone when reading threats from a restored study board', () => {
    const answer = getLocalQuestionAnswer('What am I threatening?', settledShapeGame(), 'guided');

    expect(answer?.text).toContain('This restored study position has board stones but no move history, so I cannot identify a last Black move to turn into a threat.');
    expect(answer?.text).toContain('Use the current target first: Choose a new area.');
    expect(answer?.text).toContain('Try H8 or H2.');
    expect(answer?.text).toContain('I marked the current targets so your next move can create a real plan.');
    expect(answer?.text).not.toContain('you need a Black stone');
    expect(answer?.text).not.toContain('first targets');
    expect(answer?.conceptIds).toEqual(expect.arrayContaining(['direction-of-play', 'reading', 'shape']));
    expect(answer?.boardFocus?.suggestions).toEqual([
      {
        id: 'local-threat-move-7,1',
        point: { x: 7, y: 1 },
        rank: 1,
        reason: 'Consider H8 as a fresh upper-right direction away from the settled local shape.',
      },
      {
        id: 'local-threat-move-7,7',
        point: { x: 7, y: 7 },
        rank: 2,
        reason: 'Consider H2 as a fresh lower-right direction away from the settled local shape.',
      },
  ...
  it('routes regret-style candidate questions to local target guidance', () => {
    const firstMove = playMove(createGame(9), { x: 2, y: 2 });
    if (!firstMove.success) throw new Error('test setup move failed');

    const answer = getLocalQuestionAnswer('Should I have played E7 instead?', firstMove.newState, 'guided');
  ...
  it('routes better-candidate questions to local target guidance', () => {
    const firstMove = playMove(createGame(9), { x: 2, y: 2 });
    if (!firstMove.success) throw new Error('test setup move failed');

    const answer = getLocalQuestionAnswer('Would E7 have been better?', firstMove.newState, 'guided');
  ...
  it('routes is-better candidate questions to local target guidance', () => {
    const firstMove = playMove(createGame(9), { x: 2, y: 2 });
    if (!firstMove.success) throw new Error('test setup move failed');

    const answer = getLocalQuestionAnswer('Is E7 better than my move?', firstMove.newState, 'guided');
  ...
  it('routes worth-playing candidate questions to local target guidance', () => {
    const firstMove = playMove(createGame(9), { x: 2, y: 2 });
    if (!firstMove.success) throw new Error('test setup move failed');

    const answer = getLocalQuestionAnswer('Is E7 worth playing?', firstMove.newState, 'guided');
  ...
  it('routes present-tense candidate-purpose questions to local target guidance', () => {
    const firstMove = playMove(createGame(9), { x: 2, y: 2 });
    if (!firstMove.success) throw new Error('test setup move failed');

    const answer = getLocalQuestionAnswer('What does E7 do?', firstMove.newState, 'guided');
  ...
  it('routes what-is-it-for candidate-purpose questions to local target guidance', () => {
    const firstMove = playMove(createGame(9), { x: 2, y: 2 });
    if (!firstMove.success) throw new Error('test setup move failed');

    const answer = getLocalQuestionAnswer('What is E7 for?', firstMove.newState, 'guided');

    expect(answer?.text).toContain('E7 would help the current goal: Make your stones work together.');
    expect(answer?.text).toContain('E7 is marked because it is a one-space jump from C7');
    expect(answer?.text).toContain('I marked the current targets again so you can see what E7 is doing before playing.');
    expect(answer?.text).not.toContain('Go coordinates name intersections');
    expect(answer?.text).not.toContain('Yes. E7 fits the current goal');
    expect(answer?.boardFocus?.suggestions.map((suggestion) => suggestion.point)).toEqual([
      { x: 4, y: 2 },
      { x: 2, y: 4 },
  ...
  it('routes candidate-help questions to local target guidance', () => {
    const firstMove = playMove(createGame(9), { x: 2, y: 2 });
    if (!firstMove.success) throw new Error('test setup move failed');

    const answer = getLocalQuestionAnswer('How does E7 help?', firstMove.newState, 'guided');
  ...
  it('routes White split questions to one-space jump gap pressure', () => {
    const firstMove = playMove(createGame(9), { x: 2, y: 2 });
    if (!firstMove.success) throw new Error('test setup first move failed');
    const afterWhitePass = passMove(firstMove.newState);
    const extensionMove = playMove(afterWhitePass, { x: 4, y: 2 });
    ...
    const answer = getLocalQuestionAnswer('Can White split my stones?', game, 'guided');

    expect(answer?.text).toContain('D7 is the one-point gap between C7 and E7.');
    expect(answer?.text).toContain('White can test that gap by playing D7, but that is pressure, not an immediate capture.');
    expect(answer?.text).toContain('If White actually attacks D7, count liberties before reacting');
    expect(answer?.text).not.toContain('cloud');
  ...
  it('routes fix-this-cut questions to the occupied-cut reply instead of undo', () => {
    const firstMove = playMove(createGame(9), { x: 2, y: 2 });
    if (!firstMove.success) throw new Error('test setup first move failed');
    const afterWhitePass = passMove(firstMove.newState);
    const extensionMove = playMove(afterWhitePass, { x: 4, y: 2 });
    ...
    const answer = getLocalQuestionAnswer('How do I fix this cut?', whiteCut.newState, 'guided');

    expect(answer?.text).toContain('White has played into the one-space jump gap at D7.');
    expect(answer?.text).toContain('Answer the cut by attacking the marked White liberties, starting with D8 or D6.');
    expect(answer?.text).not.toContain('The Undo button');
    expect(answer?.text).not.toContain('take back');
    expect(answer?.conceptIds).toEqual(expect.arrayContaining(['connect-and-cut', 'reading', 'liberties', 'groups']));
    expect(answer?.boardFocus?.highlights).toEqual([{
      id: 'local-occupied-cut-stone-3,2',
      point: { x: 3, y: 2 },
  ...
  it('routes still-connect questions to the occupied-cut reply', () => {
    const firstMove = playMove(createGame(9), { x: 2, y: 2 });
    if (!firstMove.success) throw new Error('test setup first move failed');
    const afterWhitePass = passMove(firstMove.newState);
    const extensionMove = playMove(afterWhitePass, { x: 4, y: 2 });
    ...
    const answer = getLocalQuestionAnswer('Can I still connect?', whiteCut.newState, 'guided');

    expect(answer?.text).toContain('White has played into the one-space jump gap at D7.');
    expect(answer?.text).toContain('C7 and E7 are separate Black groups by the rules now, but neither is captured.');
    expect(answer?.text).toContain('Answer the cut by attacking the marked White liberties, starting with D8 or D6.');
    expect(answer?.text).not.toContain('Stones become one solid group only when they touch up, down, left, or right.');
    expect(answer?.conceptIds).toEqual(expect.arrayContaining(['connect-and-cut', 'reading', 'liberties', 'groups']));
    expect(answer?.boardFocus?.highlights).toEqual([{
      id: 'local-occupied-cut-stone-3,2',
      point: { x: 3, y: 2 },
