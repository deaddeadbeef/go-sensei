...
describe('useGoMaster local answers', () => {
  ...
  it('answers move-anxiety messages with a concrete local test before fetching', () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch');
    const { result } = renderHook(() => useGoMaster());

    act(() => {
      result.current.sendMessage("I'm afraid to make a mistake");
    });
    ...
    expect(fetchSpy).not.toHaveBeenCalled();
    expect(state.bubble.variant).toBe('teaching');
    expect(state.bubble.text).toContain('A mistake is useful if it tests one clear idea.');
    expect(state.bubble.text).toContain('Your safe test is: Make your stones work together.');
    expect(state.bubble.text).toContain('Play a one-space jump from one of your stones. Try E7 or C5.');
    expect(state.bubble.text).toContain('After you play it, ask what changed');
    expect(state.bubble.actions).toEqual([{ id: 'hint', label: 'Show targets' }]);
    expect(state.chatMessages.at(-1)?.actions).toEqual([{ id: 'hint', label: 'Show targets' }]);
    expect(state.overlays.suggestions).toEqual([
      {
        id: 'local-move-anxiety-move-4,2',
        point: { x: 4, y: 2 },
        rank: 1,
        reason: 'Try E7 as a one-space jump that works with your stones.',
      },
      {
        id: 'local-move-anxiety-move-2,4',
        point: { x: 2, y: 4 },
        rank: 2,
        reason: 'Try C5 as a one-space jump that works with your stones.',
      },
  ...
  it('routes White endpoint-separate questions to one-space jump pressure without fetching', () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch');
    const { result } = renderHook(() => useGoMaster());
    ...
    act(() => {
      result.current.sendMessage('Can White separate C7 and E7?');
    });
    ...
    expect(state.bubble.text).toContain('White can test that gap by playing D7, but that is pressure, not an immediate capture.');
    expect(state.bubble.text).toContain('If White actually attacks D7, count liberties before reacting');
    expect(state.bubble.text).not.toContain('C7 and E7 are not one solid group by the rules yet.');
    expect(state.chatMessages.at(-1)?.actions).toEqual([
      { id: 'hint', label: 'Show targets' },
      { id: 'practice:reading', label: 'Practice reading' },
    ]);
  ...
  it('routes natural good-move wording to local move review without fetching', () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch');
    const { result } = renderHook(() => useGoMaster());

    act(() => {
      result.current.sendMessage('Was that a good move?');
    });
  ...
  it('routes fix-this-cut questions to the occupied-cut reply without fetching', () => {
    act(() => {
      useGameStore.getState().startGuidedIntroGame();
      useGameStore.getState().placeStone({ x: 2, y: 2 });
      useGameStore.getState().pass();
    ...
    act(() => {
      result.current.sendMessage('How do I fix this cut?');
    });
    ...
    expect(state.bubble.text).toContain('White has played into the one-space jump gap at D7.');
    expect(state.bubble.text).toContain('Answer the cut by attacking the marked White liberties, starting with D8 or D6.');
    expect(state.bubble.text).not.toContain('The Undo button');
    expect(state.bubble.text).not.toContain('take back');
    expect(state.chatMessages.at(-1)?.actions).toEqual([
      { id: 'hint', label: 'Show targets' },
      { id: 'practice:reading', label: 'Practice reading' },
    ]);
  ...
  it('routes still-connect questions to the occupied-cut reply without fetching', () => {
    act(() => {
      useGameStore.getState().startGuidedIntroGame();
      useGameStore.getState().placeStone({ x: 2, y: 2 });
      useGameStore.getState().pass();
    ...
    act(() => {
      result.current.sendMessage('Can I still connect?');
    });
    ...
    expect(state.bubble.text).toContain('C7 and E7 are separate Black groups by the rules now, but neither is captured.');
    expect(state.bubble.text).toContain('Answer the cut by attacking the marked White liberties, starting with D8 or D6.');
    expect(state.bubble.text).not.toContain('Stones become one solid group only when they touch up, down, left, or right.');
    expect(state.chatMessages.at(-1)?.actions).toEqual([
      { id: 'hint', label: 'Show targets' },
      { id: 'practice:reading', label: 'Practice reading' },
    ]);
  ...
  it('routes capture-fear questions to the occupied-cut reply without fetching', () => {
    act(() => {
      useGameStore.getState().startGuidedIntroGame();
      useGameStore.getState().placeStone({ x: 2, y: 2 });
      useGameStore.getState().pass();
    ...
    act(() => {
      result.current.sendMessage('Did White capture my stones?');
    });
    ...
    expect(state.bubble.text).toContain('White has played into the one-space jump gap at D7.');
    expect(state.bubble.text).toContain('C7 and E7 are separate Black groups by the rules now, but neither is captured.');
    expect(state.bubble.text).toContain('Black at C7 has 3 liberties: C8, C6, and B7.');
    expect(state.bubble.text).toContain('Black at E7 has 3 liberties: E8, E6, and F7.');
    expect(state.bubble.text).toContain('Answer the cut by attacking the marked White liberties, starting with D8 or D6.');
    expect(state.bubble.text).not.toContain('To capture, fill every liberty of one connected enemy group.');
    expect(state.chatMessages.at(-1)?.actions).toEqual([
      { id: 'hint', label: 'Show targets' },
      { id: 'practice:reading', label: 'Practice reading' },
    ]);
  ...
  it('routes atari-fear questions to the occupied-cut reply without fetching', () => {
    act(() => {
      useGameStore.getState().startGuidedIntroGame();
      useGameStore.getState().placeStone({ x: 2, y: 2 });
      useGameStore.getState().pass();
    ...
    act(() => {
      result.current.sendMessage('Am I in atari?');
    });
    ...
    expect(state.bubble.text).toContain('White has played into the one-space jump gap at D7.');
    expect(state.bubble.text).toContain('C7 and E7 are separate Black groups by the rules now, but neither is captured.');
    expect(state.bubble.text).toContain('Because both Black groups have more than one liberty, this is a cut, not atari yet.');
    expect(state.bubble.text).toContain('Answer the cut by attacking the marked White liberties, starting with D8 or D6.');
    expect(state.bubble.text).not.toContain('Atari means a stone or group has exactly one liberty left.');
    expect(state.chatMessages.at(-1)?.actions).toEqual([
      { id: 'hint', label: 'Show targets' },
      { id: 'practice:reading', label: 'Practice reading' },
    ]);
    ...
    expect(useConceptStore.getState().getMastery('connect-and-cut').encounterCount).toBeGreaterThan(0);
    expect(useConceptStore.getState().getMastery('atari').encounterCount).toBeGreaterThan(0);
  });
  ...
  it('routes regret-style candidate questions locally without fetching', () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch');
    const { result } = renderHook(() => useGoMaster());

    act(() => {
      result.current.sendMessage('Should I have played E7 instead?');
    });
    ...
    expect(state.bubble.text).toContain('E7 is marked because it is a one-space jump from C7');
    expect(state.bubble.text).toContain('I marked the current targets again so you can compare the options before playing.');
    expect(state.chatMessages.at(-1)?.actions).toEqual([{ id: 'hint', label: 'Show targets' }]);
    expect(state.overlays.suggestions.map((suggestion) => suggestion.point)).toEqual([
      { x: 4, y: 2 },
      { x: 2, y: 4 },
  ...
  it('routes better-candidate questions locally without fetching', () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch');
    const { result } = renderHook(() => useGoMaster());

    act(() => {
      result.current.sendMessage('Would E7 have been better?');
    });
  ...
  it('routes is-better candidate questions locally without fetching', () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch');
    const { result } = renderHook(() => useGoMaster());

    act(() => {
      result.current.sendMessage('Is E7 better than my move?');
    });
  ...
  it('routes worth-playing candidate questions locally without fetching', () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch');
    const { result } = renderHook(() => useGoMaster());

    act(() => {
      result.current.sendMessage('Is E7 worth playing?');
    });
  ...
  it('routes candidate-purpose questions locally without fetching', () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch');
    const { result } = renderHook(() => useGoMaster());

    act(() => {
      result.current.sendMessage('What would E7 do?');
    });
  ...
  it('routes present-tense candidate-purpose questions locally without fetching', () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch');
    const { result } = renderHook(() => useGoMaster());

    act(() => {
      result.current.sendMessage('What does E7 do?');
    });
  ...
  it('routes what-is-it-for candidate-purpose questions locally without fetching', () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch');
    const { result } = renderHook(() => useGoMaster());

    act(() => {
      result.current.sendMessage('What is E7 for?');
    });
    ...
    expect(state.bubble.text).toContain('E7 is marked because it is a one-space jump from C7');
    expect(state.bubble.text).toContain('I marked the current targets again so you can see what E7 is doing before playing.');
    expect(state.bubble.text).not.toContain('Go coordinates name intersections');
    expect(state.bubble.text).not.toContain('Yes. E7 fits the current goal');
    expect(state.chatMessages.at(-1)?.actions).toEqual([{ id: 'hint', label: 'Show targets' }]);
    expect(state.overlays.suggestions.map((suggestion) => suggestion.point)).toEqual([
      { x: 4, y: 2 },
